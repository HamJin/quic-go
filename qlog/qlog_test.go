	...
	Context("Events", func() {
		...
		It("records a sent packet, without an ACK", func() {
			...
			tracer.SentPacket(
				...
				nil,
				[]ackhandler.Frame{
					{Frame: &wire.MaxStreamDataFrame{StreamID: 42, ByteOffset: 987}},
					{Frame: &wire.StreamFrame{StreamID: 123, Offset: 1234, Data: []byte("foobar"), FinBit: true}},
				},
			)
		...
		It("records a sent packet, without an ACK", func() {
			tracer.SentPacket(
				...
				[]ackhandler.Frame{{Frame: &wire.MaxDataFrame{ByteOffset: 987}}},
			)
